...
class StackedCTABGANSynthesizer:
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, type={}):
        ...
        self.cond_generator = Condvec(train_data, self.transformer.output_info, self.batch_size)
        ...
        self.generator0.apply(weights_init)
        ...
        for i in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                noisez0 = torch.randn(self.batch_size, self.random_dim, device=self.device)
                # sampling conditional vectors 
                condvec0 = self.cond_generator.sample_train()
                
                c0, m0, col0, opt0 = condvec0
                c0 = torch.from_numpy(c0).to(self.device)
                m0 = torch.from_numpy(m0).to(self.device)
                ...
                noisez0 = torch.cat([noisez0, c0], dim=1)
                noisez0 =  noisez0.view(self.batch_size,self.random_dim+self.cond_generator.n_opt,1,1)
                ...
                y_real0,_ = discriminator0(real_cat_d0)
                ...
                y_fake0,_ = discriminator0(fake_cat_d0)
                ...
                loss_d0 = (-(torch.log(y_real0 + 1e-4).mean()) - (torch.log(1. - y_fake0 + 1e-4).mean()))
                ...
                noisez1 = torch.randn(self.batch_size, self.random_dim, device=self.device)
                condvec1 = self.cond_generator.sample_train()
                c1, m1, col1, opt1 = condvec1
                c1 = torch.from_numpy(c1).to(self.device)
                m1 = torch.from_numpy(m1).to(self.device)
                noisez1 = torch.cat([noisez1, c1], dim=1)
                noisez1 =  noisez1.view(self.batch_size,self.random_dim+self.cond_generator.n_opt,1,1)
                ...
                fake1 = self.generator0(noisez1)
                faket1 = self.Gtransformer.inverse_transform(fake1)
                fakeact1 = apply_activate(faket1, self.transformer.output_info)
                # concatenating conditional vectors and converting it to the image domain to be fed to the discriminator
                fake_cat1 = torch.cat([fakeact1, c1], dim=1) 
                fake_cat1 = self.Dtransformer.transform(fake_cat1)
                ...
                cross_entropy = cond_loss(faket1, self.transformer.output_info, c1, m1)
                ...
                g = -(torch.log(y_fake1 + 1e-4).mean()) + cross_entropy
                ...
                loss_mean = torch.norm(torch.mean(info_fake1.view(self.batch_size,-1), dim=0) - torch.mean(info_real1.view(self.batch_size,-1), dim=0), 1)
                loss_std = torch.norm(torch.std(info_fake1.view(self.batch_size,-1), dim=0) - torch.std(info_real1.view(self.batch_size,-1), dim=0), 1)
                ...
                if problem_type:
                    ...
                    real_pre, real_label = classifier(real0)
                    if (st_ed[1] - st_ed[0])==2:
                    ...
                    fake_pre, fake_label = classifier(fakeact1)
                    if (st_ed[1] - st_ed[0])==2:
                ...
                input_second = torch.cat([fakeact0.detach(), c0.detach()], dim=1)
                input_second =  input_second.view(self.batch_size, col_size_g+self.cond_generator.n_opt,1,1)
                
                real = data_sampler.sample(self.batch_size, col0[perm], opt0[perm])
                real = torch.from_numpy(real.astype('float32')).to(self.device)

                # storing shuffled ordering of the conditional vectors
                c_perm0 = c0.detach()[perm]
                
                # generating synthetic data as an image
                fake = self.generator1(input_second)
                ...
                # computing the probability of the discriminator to correctly classify real samples hence y_real should ideally be close to 1
                y_real,_ = discriminator1(real_cat_d)
                # computing the probability of the discriminator to correctly classify fake samples hence y_fake should ideally be close to 0
                y_fake,_ = discriminator1(fake_cat_d)
                ...
                # computing the probability of the discriminator classifiying fake samples as real 
                # along with feature representaions of fake data resulting from the penultimate layer 
                y_fake,info_fake = discriminator1(fake_cat)
                # extracting feature representation of real data from the penultimate layer of the discriminator 
                _,info_real = discriminator1(real_cat_d)
                # computing the conditional loss to ensure the generator generates data records with the chosen category as per the conditional vector
                cross_entropy = cond_loss(faket, self.transformer.output_info, c1, m1)
                ...
                print(str(torch.log(y_fake + 1e-4).mean()))
    ...
    def sample(self, n):
        ...
        for _ in range(steps):
            ...
            condvec = self.cond_generator.sample()
            c = condvec
            c = torch.from_numpy(c).to(self.device)
            noisez = torch.cat([noisez, c], dim=1)
            noisez =  noisez.view(self.batch_size,self.random_dim+self.cond_generator.n_opt,1,1)
            fake0 = self.generator0(noisez)
            faket0 = self.Gtransformer.inverse_transform(fake0)
            fakeact0 = apply_activate(faket0,output_info)
            
            
            
            input_second = torch.cat([fakeact0, c], dim=1) 
            input_second = input_second.view(self.batch_size, self.transformer.output_dim+self.cond_generator.n_opt,1,1)
            
            fake1 = self.generator1(input_second)
            faket1 = self.Gtransformer.inverse_transform(fake1)
            fakeact1 = apply_activate(faket1, output_info)
            
            data_first_gen.append(fakeact0.detach().cpu().numpy())
            data_stacked.append(fakeact1.detach().cpu().numpy())

        data_first_gen = np.concatenate(data_first_gen, axis=0)
        ...
        result_first_gen = self.transformer.inverse_transform(data_first_gen)
        result_stacked = self.transformer.inverse_transform(data_stacked)
        
        return result_first_gen[0:n], result_stacked[0:n]
